#!/usr/bin/env python3
"""
Segment clinical text, search PubMed-grounded clinical citation candidates, enrich DOI
...
USER_AGENT = "codex-clinical-citation/1.0 (mailto:unknown@example.com)"
EXPORT_FORMAT_CHOICES = ("enw", "ris", "zotero-rdf", "rdf")
DEFAULT_EXPORT_FORMAT = "enw"
SCOPE_CHOICES = ("big4", "jama", "nejm", "lancet", "bmj", "gi", "big4-gi", "major-clinical")
...
def export_filename(export_format: str, base: str = "clinical_references") -> str:
...
def journal_family(journal: str) -> str:
    ...
    if journal_norm in JOURNAL_TO_FAMILY:
        ...
        return "major-clinical" if family == "major-extra" else family
...
def request_json(url: str, mailto: str | None = None) -> dict[str, Any]:
    headers = {"User-Agent": USER_AGENT if not mailto else f"codex-clinical-citation/1.0 (mailto:{mailto})"}
...
def build_ris_record(item: Candidate) -> str:
    ...
    lines.append("N1  - Clinical candidate is metadata/abstract-screened only.")
    lines.append("ER  -")
    return "\n".join(lines)
...
def write_artifacts(mapping: list[dict[str, Any]], references: list[Candidate], outdir: Path, output_path: Path, args: argparse.Namespace, errors: list[dict[str, str]]) -> None:
    stem = output_path.stem or "clinical_references"
    payload = {
        "scope": args.scope,
        ...
        "notes": [
            ...
            "Vancouver/AMA style is the default clinical reporting style.",
        ],
    }
    ...
    (outdir / f"{stem}.html").write_text(
        "<!doctype html><html><head><meta charset='utf-8'><title>Clinical Citation Report</title></head><body>"
        "<h1>Clinical Citation Report</h1><p>Screening status: metadata/abstract-screened only.</p>"
        + "\n".join(html_cards)
        + "</body></html>",
        encoding="utf-8",
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PubMed-grounded clinical citation search and export.")
    parser.add_argument("--text", action="append", help="Clinical manuscript text to segment. Can be repeated.")
    parser.add_argument("--text-file", help="UTF-8 text file.")
    parser.add_argument("--claim", action="append", help="Single clinical claim. Can be repeated.")
